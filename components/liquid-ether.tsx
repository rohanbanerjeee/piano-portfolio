...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class MouseClass {
      ...
      init(container: HTMLElement, useDocumentEvents = false) {
        this.container = container
        const target = useDocumentEvents ? document : container
      ...
      dispose() {
        if (!this.container) return
        // Remove from both document and container to be safe
...
`
    ...
    class ShaderPass {
      ...
      plane: THREE.Mesh<THREE.PlaneGeometry, THREE.RawShaderMaterial> | null = null
      ...
      update() {
        ...
        if (this.scene && this.camera && Common.renderer) {
    ...
    class WebGLManager {
      ...
      constructor(props: any) {
        ...
        Mouse.init(props.$wrapper, !props.autoDemo)
  ...
  return (
    <div
      ref={mountRef}
      className={`w-full h-full relative overflow-hidden ${!autoDemo ? 'pointer-events-auto' : 'pointer-events-none'} touch-none ${className || ""}`}
      style={style}
    />
  )
}
